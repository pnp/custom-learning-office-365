...
if ($AppCatalogAdmin) {   
    try {
        ...
        Connect-PnPOnline $clSite -Credentials $Credentials
  ...
  $appcatalog = Get-PnPTenantAppCatalogUrl # get the pnp app catalog but then connect to the app catalog
  Disconnect-PnPOnline

  Connect-PnPOnline $appcatalog -Credentials $Credentials
